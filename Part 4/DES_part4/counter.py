...
class TimeIndependentCrosscorrelationCounter(TimeIndependentCounter):
    ...
    def get_cov(self):
        ...
        """
        # TODO Task 4.1.1: Your code goes here
        dot = numpy.multiply(self.x.values,self.y.values)
        xy=numpy.mean(dot)
        xm= self.x.get_mean()
        ym= self.y.get_mean()
        return xy - xm*ym # E[XY]-E[X]E[Y]
    ...
    def get_cor(self):
        ...
        varX= self.x.get_var()
        varY = self.y.get_var()
        return self.get_cov()/math.sqrt(varX*varY)
...
class TimeIndependentAutocorrelationCounter(TimeIndependentCounter):
    ...
    def __init__(self, name="default", max_lag=10):
        ...
        self.x= TimeIndependentCounter()
        self.shifted=TimeIndependentCounter()
    ...
    def get_auto_cov(self, lag):
        ...
        """
        # TODO Task 4.1.2: Your code goes here
        self.shifted.values=numpy.roll(self.x.values,lag)
        dot=numpy.multiply(self.x.values,self.shifted.values)
        xy=numpy.mean(dot)
        xm=self.x.get_mean()
        ym=self.shifted.get_mean()
        return xy-xm*ym
    ...
    def get_auto_cor(self, lag):
        ...
        self.shifted.values=numpy.roll(self.x.values,lag)
        varX = self.x.get_var()
        varY = self.shifted.get_var()
        return self.get_auto_cov(lag)/math.sqrt(varX*varY)
